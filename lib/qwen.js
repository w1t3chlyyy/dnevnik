...
export async function askQwenRaw(systemPrompt, userPrompt) {
  const messages = [
    { role: "system", content: systemPrompt },
    { role: "user", content: userPrompt }
  ];
  const response = await callQwen(messages, { toolsOverride: [], toolChoiceOverride: "none" });
  const choice = response.choices[0];
  return choice.message.content;
}

async function callQwen(messages, options = {}) {
  const { toolsOverride = tools, toolChoiceOverride = "auto" } = options;
