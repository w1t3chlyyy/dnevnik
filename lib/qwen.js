...
const tools = [
  ...
  },
  {
    ...
    function: {
      name: "delete_goal",
      description:
        "Удалить обычную цель по названию. Цель скрывается из мини-аппа, но её прошлый прогресс " +
        "не стирается из статистики (get_summary) — удаление необратимо только в интерфейсе.",
...
async function runTool(user, name, args) {
  try {
    ...
    if (name === "delete_goal") {
      ...
      // goal_progress, и прошлые периоды в get_summary/"Итогах" задним
      // числом "похудеют". Вместо этого помечаем hidden=true: цель пропадает
      // из мини-аппа и списков, но продолжает учитываться в статистике.
      const { data: goal } = await db
        .from("goals")
        .select("id")
        .eq("user_id", user.id)
