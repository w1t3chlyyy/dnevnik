...
// Просим Qwen оценить КБЖУ по текстовому описанию еды. Формат ответа жёстко
// задан промптом, но LLM иногда добавляет пояснения вокруг JSON — поэтому
// парсим через regex, а не JSON.parse(raw) напрямую.
export async function estimateFoodMacros(user, description) {
  const prompt =
    `Ты — нутрициолог-калькулятор калорий. Пользователь описал приём пищи: "${description}".\n` +
    `Оцени суммарную калорийность и БЖУ на ВЕСЬ описанный приём пищи (сумма всех продуктов), ` +
    `основываясь на типичных порциях и составе продуктов.\n` +
    `Ответь СТРОГО одним JSON-объектом без пояснений, без markdown-разметки, вот в таком формате:\n` +
    `{"calories": число, "protein": число, "fat": число, "carbs": число}\n` +
    `calories — ккал, остальное — граммы. Только число, без единиц измерения внутри значений.`;

  const raw = await askQwen(user, prompt);
