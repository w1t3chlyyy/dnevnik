...
// текстовому описанию делает Qwen (estimateFoodMacros), с ручным fallback,
// если модель не смогла вернуть валидный JSON.
...
bot.callbackQuery(/^nutr_goal:(lose|maintain|gain)/, async (ctx) => {
  ...
  const { error } = await db.from("nutrition_profiles").upsert({
    user_id: user.id,
