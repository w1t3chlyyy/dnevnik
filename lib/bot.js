...
bot.on("message:text", async (ctx) => {
  ...
  if (step === "progress_value") {
    ...
    const completed = goalRow && total >= Number(goalRow.target_value);
        const updatePatch = { current_value: total, status: completed ? "done" : "active" };
  ...
  if (step === "dgoal_unit") {
    ...
    const kb = new InlineKeyboard()
      ...
      .row()
      .text("🏠 Меню", "menu:home");
    return ctx.reply(`Цель на сегодня зафиксирована 🎯 Удачи! Можешь добавить ещё одну.${linkNote}`, {
      reply_markup: kb
    });
  ...
  if (step === "dgoal_progress_value") {
    ...
    const updPatch = { current_value: total, status: completed ? "done" : "active" };
    ...
    let extra = "";
    if (completed && goalRow.status !== "done") {
