...
// без истории чата), с ручным fallback, если модель не смогла вернуть
// валидный JSON. Записи в food_logs/nutrition_profiles идут через
// withSupabaseRetry — спасает от разовых Gateway Timeout на стороне Supabase
// (например, при пробуждении бесплатного проекта после паузы).
...
bot.on("message:text", async (ctx) => {
  ...
  if (step === "food_desc") {
    ...
    let macros;
    try {
      macros = await estimateFoodMacros(user, description);
    } catch (e) {
      console.error("estimateFoodMacros failed:", e.message);
      ctx.session.draft.description = description;
      ctx.session.step = "food_manual_macros";
      return ctx.reply(
        "Не смог посчитать автоматически. Введи вручную одной строкой через пробел: " +
          "калории белки жиры углеводы\nНапример: 450 30 15 40"
      );
    }
    ...
    return ctx.reply(
      `Записал ✅ ${mealLabel(mealType)}: ${description}\n` +
        `🔥 ${macros.calories} ккал · 🥩 ${macros.protein} г · 🥑 ${macros.fat} г · 🍞 ${macros.carbs} г\n\n` +
  ...
  if (step === "food_manual_macros") {
    const parts = ctx.message.text
      .trim()
      .split(/\s+/)
      .map((n) => Number(n.replace(",", ".")));
    if (parts.length < 4 || parts.some((n) => Number.isNaN(n))) {
      return ctx.reply("Нужно 4 числа через пробел: калории белки жиры углеводы");
    }
    const [calories, protein, fat, carbs] = parts;
    const mealType = ctx.session.draft.meal_type;
    const description = ctx.session.draft.description || "приём пищи";
